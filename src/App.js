...
// ── Gist API ───────────────────────────────────────────────────────────────────
const GIST_FILE       = "flowchart-builder-data.json";
const TEAM_GIST_FILE  = "flowchart-builder-team.json";
const LS_TOKEN        = "fc:token";
const LS_GIST_ID      = "fc:gist_id";
...
const findGist    = async(t,file) => {
  ...
  while(true){
    ...
    const found=data.find(g=>g.files&&g.files[file]);
...
function Field({label,value,onChange,placeholder,autoFocus,type="text",hint}){
  return(
    <div style={{marginBottom:12}}>
      ...
      <input autoFocus={autoFocus} value={value} placeholder={placeholder} type={type} onChange={e=>onChange(e.target.value)}
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>
        {SWATCHES.map(s=><div key={s} onClick={()=>onSelect(s)} style={{width:22,height:22,borderRadius:5,background:s,cursor:"pointer",flexShrink:0,boxSizing:"border-box",transition:"transform 0.1s",border:current===s?"3px solid #1e3a5f":"1.5px solid #ccc"}} onMouseEnter={e=>e.currentTarget.style.transform="scale(1.25)"} onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}/>)}
      </div>
              ...
              return(
                <g key={node.id} className="ne" onPointerDown={e=>onNodeDown(e,node.id)} onContextMenu={e=>openEdit(e,node.id)} style={{cursor:"grab"}}>
                  ...
                  <g onPointerDown={e=>e.stopPropagation()} onClick={e=>onUrlClick(e,node.id)} style={{cursor:"pointer"}}>
                    <rect x={node.x+NW-30} y={node.y+8} width={24} height={24} rx={6} fill={node.url?accentColor:"#e8f0fe"} stroke={node.url?darken(accentColor):"#b3d4f5"} strokeWidth={1}/>
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:14,padding:24,width:400,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 8px 40px #0004"}}>
            {(modal.mode==="edit"||modal.mode==="url")&&<>
              ...
              <div style={{display:"flex",borderBottom:"2px solid #e8f0fe",marginBottom:14}}>
                {["info","color"].map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"6px 18px",border:"none",borderBottom:tab===t?`2px solid ${accentColor}`:"2px solid transparent",background:"transparent",color:tab===t?accentColor:"#888",fontWeight:600,fontSize:13,cursor:"pointer",marginBottom:-2}}>{t==="info"?"📝 Info":"🎨 Color"}</button>)}
              </div>
              {tab==="info"&&<><Field label="Folder Name" value={form.label} onChange={v=>setForm(f=>({...f,label:v}))} autoFocus/><Field label="URL" value={form.url} onChange={v=>setForm(f=>({...f,url:v}))} placeholder="https://..."/><BtnRow><Btn v="danger" onClick={()=>setModal({id:modal.id,mode:"delete"})}>🗑</Btn><Btn v="outline" onClick={()=>{setForm({label:"New Folder",url:""});setModal({id:modal.id,mode:"add"});}}>＋ Child</Btn><Btn v="primary" onClick={saveModal}>Save</Btn></BtnRow></>}
...
export default function App(){
  ...
  const connectTeamSpace=async()=>{
    if(!newTeamToken.trim())return;
    setSyncing(true);
    try{
      await verifyToken(newTeamToken.trim());
      localStorage.setItem(LS_TEAM_TOKEN,newTeamToken.trim());
      setTeamToken(newTeamToken.trim());
      const{data}=await getOrCreateGist(newTeamToken.trim(),"team",[],"Flowchart Builder – Team Charts");
      setTeamCharts(data);
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f1e35 0%,#1e3a5f 50%,#1a4a7a 100%)",fontFamily:"'Segoe UI',sans-serif"}}>
      ...
      <div style={{padding:"36px 32px 20px",textAlign:"center"}}>
        ...
        <button onClick={()=>setShowNew(true)}
          style={{background:activeTab==="team"?"linear-gradient(135deg,#8e44ad,#6c3483)":"linear-gradient(135deg,#4a90d9,#27ae60)",border:"none",color:"#fff",borderRadius:12,padding:"10px 28px",fontSize:14,fontWeight:700,cursor:"pointer",boxShadow:"0 4px 20px #0003"}}>
      ...
      {showNew&&(
        <div style={{position:"fixed",inset:0,background:"#0008",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}} onClick={()=>setShowNew(false)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:16,padding:28,width:420,boxShadow:"0 8px 40px #0005"}}>
            <h3 style={{margin:"0 0 4px",color:"#1e3a5f",fontSize:18}}>🆕 New Flowchart</h3>
            <p style={{margin:"0 0 16px",fontSize:12,color:activeTab==="team"?"#8e44ad":"#4a90d9",fontWeight:600}}>{activeTab==="team"?"🟣 Adding to Team Space":"👤 Adding to My Space"}</p>
            ...
            <div style={{marginBottom:18}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {TEMPLATES.map(t=>(
                  <div key={t.label} onClick={()=>setTmpl(t)} style={{padding:"10px 12px",borderRadius:10,cursor:"pointer",border:`2px solid ${tmpl.label===t.label?"#4a90d9":"#e0e8f0"}`,background:tmpl.label===t.label?"#e8f0fe":"#f9fafb",transition:"all 0.1s"}}>
